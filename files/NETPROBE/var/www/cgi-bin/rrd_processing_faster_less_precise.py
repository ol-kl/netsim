#!/usr/bin/python3.3
# Check the difference between rrd_processing.py and 
# rrd_processing_C_faster.py modules in a comment 
# section of the former
'''
Created on 28.08.2012
Last modified on 14.10.2012
...
#            return pool[set_id] 
    ...
    def fetchDB(self, stime, etime, cf, cdpToShow):
        ...
        try:
            ...
            self.err_send(6, 'rrdtool call failed. Report: ' + err.errno)
        ...
        rootElement = ET.XML(xml_file) #@UndefinedVariable
        self.lastQueryTime = int(time.time())
    ...
    def concatenateRRAs(self, rootElement):
        """
        ...
        lastTime = int(rootElement.findtext('lastupdate'))
        rra_arr = rootElement.findall('rra')
        if len(rra_arr) == 0:
            self.err_send(6,'RRA weren\'t found')
        
        DB_vals = [float(elem.text) for elem in rra_arr[0].getiterator('v')]
        pdp_per_row = int(rra_arr[0].findtext('pdp_per_row'))
        DB_tstps = [(lastTime - pdp_per_row + 1) - self.step * mult * pdp_per_row for mult in range(len(rra_arr[0].findall('.//v')))]
        DB_tstps.reverse()
        
        #time granularity of RRAs must get coarser in top-down order - it's a main assumption
        for num in range(1, len(rra_arr)):
            rra_values =  [float(elem.text) for elem in rra_arr[num].getiterator('v')]
            pdp_per_row = int(rra_arr[num].findtext('pdp_per_row'))
            rra_timestamps = [(lastTime - pdp_per_row + 1) - self.step * mult * pdp_per_row for mult in range(len(rra_arr[num].findall('.//v')))]
            # Here rra_timestamps in decreasing order, not increasing !!! 
